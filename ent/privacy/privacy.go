...
// The PlaneQueryRuleFunc type is an adapter to allow the use of ordinary
// functions as a query rule.
type PlaneQueryRuleFunc func(context.Context, *ent.PlaneQuery) error

// EvalQuery return f(ctx, q).
func (f PlaneQueryRuleFunc) EvalQuery(ctx context.Context, q ent.Query) error {
	if q, ok := q.(*ent.PlaneQuery); ok {
		return f(ctx, q)
	}
	return Denyf("ent/privacy: unexpected query type %T, expect *ent.PlaneQuery", q)
}

// The PlaneMutationRuleFunc type is an adapter to allow the use of ordinary
// functions as a mutation rule.
type PlaneMutationRuleFunc func(context.Context, *ent.PlaneMutation) error

// EvalMutation calls f(ctx, m).
func (f PlaneMutationRuleFunc) EvalMutation(ctx context.Context, m ent.Mutation) error {
	if m, ok := m.(*ent.PlaneMutation); ok {
		return f(ctx, m)
	}
	return Denyf("ent/privacy: unexpected mutation type %T, expect *ent.PlaneMutation", m)
}
